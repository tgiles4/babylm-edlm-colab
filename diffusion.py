...
class Diffusion(L.LightningModule):
  ...
  def _ddpm_caching_update(self, x, t, dt, p_x0=None):
    ...
    assert sigma_s.ndim == 1

    # Compute move_chance from sigma (works for all noise types)
    # For loglinear: move_chance ≈ t (when eps is small)
    # For cosine/other: move_chance = 1 - exp(-sigma)
    if self.config.noise.type == 'loglinear':
      # Loglinear has move_chance = (1 - eps) * t ≈ t
      move_chance_t = t[:, None, None]
      move_chance_s = (t - dt)[:, None, None]
    else:
      # For other noise types, compute from sigma
  ...
  def _reconstruction_loss(self, x0):
    t0 = torch.zeros(x0.shape[0], dtype=self.dtype,
                     device=self.device)
    # Note: This was originally for d3pm parameterization with loglinear noise
    # but works with any noise type - just gets noise at t=0
...
class EBM(Diffusion):
  ...
  def ebm_forward(self, xt, sigma, x0=None, log_p_x0=None, attention_mask=None):
    ...
    with torch.amp.autocast('cuda', dtype=torch.float32):
      ...
      if self.config.ebm_backbone == 'dit' or self.config.ebm_backbone == 'hf_dit':
        # Clamp indices to valid range for EBM vocab_embed to prevent index out of bounds
        # This is critical when x0 is sampled from a model with different vocab_size
        # (e.g., when ebm_backbone is 'hf_dit' loaded from pretrained model)
        # EmbeddingLayer uses self.embedding, not self.weight
        if hasattr(self.ebm.vocab_embed, 'embedding'):
          ebm_vocab_size = self.ebm.vocab_embed.embedding.shape[0]
        elif hasattr(self.ebm.vocab_embed, 'weight'):
          ebm_vocab_size = self.ebm.vocab_embed.weight.shape[0]
        else:
          # Fallback: use the model's vocab_size
          ebm_vocab_size = self.ebm.vocab_size if hasattr(self.ebm, 'vocab_size') else self.vocab_size
        indices = torch.clamp(indices, 0, ebm_vocab_size - 1)
        xt = self.ebm.vocab_embed(indices)
        # x0 should always be provided, but clamp to be safe
        assert x0 is not None, "x0 must be provided for EBM forward pass"
        x0 = torch.clamp(x0, 0, ebm_vocab_size - 1)
