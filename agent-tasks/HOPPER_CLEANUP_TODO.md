...
- [x] Move Colab-only files into **colab/** subfolder:
  - `install_colab.sh`
  - `install_colab.py`
...
- [x] Add **colab/COLAB.md** that explains:
...
- [x] Optionally add a short **"Colab (optional)"** section in the main **README.md** linking to `colab/COLAB.md`.

---

## 3. Docs

- [x] Add **HOPPER_SETUP.md** (or a cluster section in README) with:
...
- [x] **Consolidate Colab docs:** Merge or trim so Colab is one optional path:
...
- [x] Add a **Hopper SLURM job script** for BabyLM (e.g. **scripts/job_train_babylm_hopper.slurm** or **scripts/slurm_train_babylm.sh**):
...
- [x] In **train_tokenizer.py**:
  - **project_dir** is an argparse argument (`--project_dir`); Hopper sbatch scripts set it (e.g. `$SCRATCH/babylm-edlm`). Default when unset: `$SCRATCH/babylm-edlm` or `./scratch/babylm-edlm`.
  - Default paths are derived from project_dir and `--size`: `project_dir/data/train_{size}` and `project_dir/tokenizer/tokenizer_{size}.json` (10M and 100M use separate tokenizers).
  - Docstring and help use Hopper/generic paths; Colab/custom usage is documented in **colab/COLAB.md**.
